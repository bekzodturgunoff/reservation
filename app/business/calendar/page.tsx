...
export default function BusinessCalendar() {
  ...
  const { data: monthBookings = [] } = useQuery({
    queryKey: ['business-calendar', profile?.id, year, month],
    queryFn: async () => {
  ...
  const { data: blockedDates = [] } = useQuery({
    queryKey: ['blocked-dates', profile?.id, year, month],
    queryFn: async () => {
      const { data } = await supabase
        .from('blocked_dates')
        .select('date')
  ...
  const blockMutation = useMutation({
    mutationFn: async (date: Date) => {
      await supabase.from('blocked_dates').insert({
        venue_id: venueIds[0], date: dateStr(date), created_by: profile!.id,
      })
    },
  ...
  const dayBookings = selectedDay ? bookingsByDate[dateStr(selectedDay)] || [] : []
