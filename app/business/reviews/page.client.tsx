...
interface BusinessReviewsClientProps {
  userId: string
  initialVenues: { id: string; name: string }[]
  initialReviews: ReviewItem[]
}

export default function BusinessReviewsClient({ userId, initialVenues, initialReviews }: BusinessReviewsClientProps) {
  ...
  const { data: reviews = [], isLoading } = useQuery<ReviewItem[]>({
    ...
    queryFn: async () => {
      ...
      return (data || []) as never as ReviewItem[]
    },
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {reviews.map((r: ReviewItem) => {
            const sentimentLabel = r.rating >= 4 ? 'Yaxshi' : r.rating >= 3 ? 'O\'rtacha' : 'Yomon'
            const sentimentColors = r.rating >= 4
              ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
              : r.rating >= 3
                ? 'bg-amber-50 text-amber-700 border-amber-200'
                : 'bg-red-50 text-red-700 border-red-200'
            return (
              <Card key={r.id} className="p-5 transition-shadow hover:shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold border ${sentimentColors}`}>
                          {sentimentLabel}
                        </span>
                      </div>
                      <p className="text-xs text-ink-muted">{r.venues?.name} · {new Date(r.created_at).toLocaleDateString('uz-UZ', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                    </div>
                  </div>
