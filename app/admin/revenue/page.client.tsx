...
interface AdminRevenueClientProps {
  initialBookings: { total_price: number; status: string; created_at: string }[]
...
export default function AdminRevenueClient({
  ...
  const { data: premiumCount = 0 } = useQuery({
    ...
    initialData: initialPremiumCount,
  })
  ...
  const monthlyRevenue = Object.entries(monthlyMap).map(([key, amount]) => ({
    month: monthNames[parseInt(key.split('-')[1] ?? '0') - 1] ?? key,
