...
export function SearchAutocomplete({ value, onChange, onClear }: SearchAutocompleteProps) {
  ...
  const select = (s: Suggestion) => {
    setOpen(false)
    if (s.type === 'venue') {
      router.push(ROUTES.VENUE(s.label.toLowerCase().replace(/\s+/g, '-')))
    } else if (s.type === 'city') {
      router.push(`${ROUTES.SEARCH}?city=${encodeURIComponent(s.label)}`)
    } else if (s.type === 'category') {
      router.push(`${ROUTES.SEARCH}?category=${s.slug}`)
    }
  }
